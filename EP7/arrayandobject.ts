...
const friendsMoreThan = friendsArray.filter(function(element, index){
    return element.age > 14 && element.height > 170
})

console.log(friendsMoreThan)
...
}).map(function(element, index) { //สร้างarray และ value ตามเงื่อนไขที่เรากำหนด 
    return({
        province : element.province,
        total_death : element.total_death
    })
}).filter(function(element, index){ //คัดกรองสมาชิกตามเงื่อนไขที่เรากำหนัด
    return element.province.includes('นคร')
})
